...
var ApiGateway = (function () {
    ...
    ApiGateway.getEventValue = function (event, name) {
        var info = name.split('.');
        // Ugly hack I know.
        if (info.length == 1) {
            return event[name];
        ...
        if (info.length == 2) {
            return event[info[0]][info[1]];
        }
        if (info.length == 3) {
            return event[info[0]][info[1]][info[2]];
        }
        if (info.length == 4) {
            return event[info[0]][info[1]][info[2]][info[3]];
        }
        if (info.length == 5) {
            return event[info[0]][info[1]][info[2]][info[3]][info[4]];
        }
        if (info.length == 6) {
            return event[info[0]][info[1]][info[2]][info[3]][info[4]][info[5]];
        }
    };
